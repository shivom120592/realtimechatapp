...
$("#file-input").change(() => {
	...
	let selectedImage = $("#file-input").val();
	fileReader.onload = function (oFREvent) {
		document.getElementById('show-image').src = fileReader.result;
	}
})
...
const uploadImage = () => {
	let mf = $("#file-input").prop('files')[0];
	...
	$.ajax({
		...
		success: function (e) {
			...
			document.getElementById('show-image').src = 'https://goo.gl/pB9rpQ';
					...
					if (change.doc.data().user_1_uuid == user_uuid) {
						if (change.doc.data().image != undefined) {
							newMessage += '<div class="message-block">' +
								'<div class="user-icon"></div>' +
								'<div class="message"> <p>' + change.doc.data().message + '</p> <img style=\'height: 80%; width: 30%;\' src=\'' + change.doc.data().image + '\'>' + '</div>' +
								'</div>';
						} else {
							newMessage += '<div class="message-block">' +
								'<div class="user-icon"></div>' +
								'<div class="message"><p>' + change.doc.data().message + '</p></div>' +
								'</div>';
						}
